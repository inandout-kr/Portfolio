...
> **방향 전환 메모 (2026-06):** 사용자의 원래 요구는 "알고리즘 트레이딩에 특화된 *새 프로그래밍 언어*"였다. §11-3에서 이를 'Python 임베디드 DSL'로 구현했으나, 이는 진짜 언어가 아니라 라이브러리였다 → 사용자 재확인 후 **독립 언어로 재구현 시작**. 트리워킹 인터프리터(`kairoslang/`, 자체 문법 `.ka`). 발굴 시스템(아래 `kairos_*.py`)은 **보류**. 언어 진행 상황·문법·로드맵은 `kairoslang/README.md` 참조. 아래 내용은 보류된 발굴 시스템 브리프다.
